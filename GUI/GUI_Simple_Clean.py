...
def show_stats():
    ...
    # Get display mode for title
    display_mode = display_var.get()
    if display_mode == "Single Month":
        mode_text = f" ({month_var.get()})"
    else:
        mode_text = " (All Data)"
    
    stats_info = f"STATISTICS - {variable}{mode_text}\n"
    stats_info = stats_info + "="*40 + "\n\n"
    stats_info = stats_info + f"Total rows in dataset: {len(data)}\n"
    stats_info = stats_info + f"Valid data points: {len(clean_data)}\n"
    stats_info = stats_info + f"Missing/invalid data: {len(data) - len(clean_data)}\n\n"
...
def setup_gui():
    ...
    global root, main_frame, control_frame, display_frame, display_var
    global month_frame, month_var, var_frame, variable_var, plot_var
    global button_frame, notebook, plot_frame, stats_frame, fig, ax, canvas, stats_text
