...
bomfile = os.path.join(refined_datasets_dir, 'bom_5factor.dat')
house4file = os.path.join(refined_datasets_dir, 'house4_5factor.dat')
...
def process_bom_data():
    ...
    for line in bomarr:
        ...
        data = []
        flag = True
          # Load columns: MinTemp (index 1), MaxTemp (index 2), Rainfall (index 3), 9am temp (index 4), 3pm temp (index 5)
        temp_indices = [1, 2, 3, 4, 5]  # MinTemp, MaxTemp, Rainfall, 9amTemp, 3pmTemp
        ...
        if flag and len(data) == 5:  # Ensure we have all 5 features
            bom[key] = data
...
def generate_training_data(bom, house4data_processed):
    """Generate full dataset for ML - FIVE factor learning: weather features - power"""
    print("Generating training data...")
    
    x_train_full = []
    y_train_full = []    # Five factor: MinTemp, MaxTemp, Rainfall, 9amTemp, 3pmTemp
    for k in bom.keys():
        k1 = k + '09'  # 9am data
        k2 = k + '15'  # 3pm data
        ...
        if k1 in keys:
            # Use all 5 weather features for 9am energy prediction
            if len(bom[k]) >= 5:
                x_train_full.append([bom[k][0], bom[k][1], bom[k][2], bom[k][3], bom[k][4]])  # MinTemp, MaxTemp, Rainfall, 9amTemp, 3pmTemp
                y_train_full.append(house4data_processed[k1])
            else:
                print(f"Warning: BOM data for {k} has only {len(bom[k])} values, expected 5")
        
        if k2 in keys:
            # Use all 5 weather features for 3pm energy prediction
            if len(bom[k]) >= 5:
                x_train_full.append([bom[k][0], bom[k][1], bom[k][2], bom[k][3], bom[k][4]])  # MinTemp, MaxTemp, Rainfall, 9amTemp, 3pmTemp
...
def create_train_test_split(x_train_full, y_train_full):
    ...
    x_train = x_train.reshape(len(x_train), 1, 5)  # 5 weather features
    x_test = x_test.reshape(len(x_test), 1, 5)
...
def create_rnn_model():
    ...
    with tf.device(device_name):
        ...
        model.add(tf.keras.layers.SimpleRNN(
            ...
            input_shape=(1, 5),  # 5 weather features
...
def run_incremental_epoch_comparison():
    ...
    for i, epochs in enumerate(epoch_tests, 1):
        ...
        save_epoch_results_to_csv(epochs, y_test, predictions, mse, error_rate, i == 1)  # header only on first iteration      # Create 5x2 subplot visualization (10 plots total) removed - now handled by plot_refined_data.py
    ...
    summary_file = os.path.join(refined_datasets_dir, 'incremental_epoch_comparison_summary_5factor.csv')
...
def save_epoch_results_to_csv(epochs, y_test, predictions, mse, error_rate, write_header=False):
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    refined_datasets_dir = os.path.join(script_dir, 'Refined Datasets')
    csv_file = os.path.join(refined_datasets_dir, 'incremental_epoch_results_5factor.csv')
...
def save_difference_results_to_csv(epoch_from, epoch_to, prediction_diff, mean_diff, std_diff, write_header=False):
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    refined_datasets_dir = os.path.join(script_dir, 'Refined Datasets')
    csv_file = os.path.join(refined_datasets_dir, 'epoch_differences_results_5factor.csv')
...
def main():
    ...
    print("Using 5 weather features (Min Temp, Max Temp, Rainfall, 9am Temp, 3pm Temp) to predict energy consumption")
    print()
